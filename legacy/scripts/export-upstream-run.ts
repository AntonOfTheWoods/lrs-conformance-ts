...
function usage(): string {
  return [
    "Usage:",
    "  bun run legacy:export:upstream:lrsql -- [--suite-dir <path>] [--provided-suite-runner-mode native] [--base-url <url>] [--username <user>] [--password <pass>] [--version 2.0.0|1.0.3] [--out <path>] [--grep <pattern>] [--directory <csv>] [--optional <csv>] [--unitKey <csv>] [--log-dir <path>] [--node-image <ref>] [--bun-image <ref>] [--upstream-repo-url <url>] [--upstream-ref <ref>] [--clone-depth <n>] [--clone-base-dir <path>] [--keep-clone]",
    "",
    "Defaults:",
    ...
    "  With --suite-dir, the local suite source is used as the active migration runtime.",
...
function resolveBunRuntimeConsoleRunnerRelativePath(suiteDir: string): string {
...
function prepareProvidedSuite(config: ExportUpstreamConfig): SuiteLocation {
  ...
  const consoleRunnerPath = resolve(suiteDir, resolveBunRuntimeConsoleRunnerRelativePath(suiteDir));
  if (!existsSync(consoleRunnerPath)) {
...
function buildRuntimeBunSuiteBootstrapCommand(
  upstreamArgs: string[],
  logDirInContainer: string,
  suiteSourceDirInContainer: string,
  ...
  return [
    ...
    '  echo "[conformance] runtime suite is missing expected files in $suite_source_dir"',
    "  exit 1",
    "fi",
    'rm -rf "$runtime_prefix_dir"',
    'mkdir -p "$runtime_suite_dir"',
...
function runRuntimeBunConsoleRunnerInContainer(config: ExportUpstreamConfig, suiteLocation: SuiteLocation): number {
  ensurePodmanAvailable();

  const { captureUnit, consoleRunnerArgs } = buildConsoleRunnerArgs(config);
  const hostBunBinaryPath = process.execPath;
  const consoleRunnerRelativePath = resolveBunRuntimeConsoleRunnerRelativePath(suiteLocation.suiteDir);

  if (!existsSync(hostBunBinaryPath)) {
    throw new Error(`Unable to find the host Bun binary for the runtime bun-ts lane: ${hostBunBinaryPath}`);
  }

  mkdirSync(config.logDir, { recursive: true });

  const logDirInContainer = posix.resolve(repoMountTarget, toPosixRelativePath(relative(repoRoot, config.logDir)));
  const suiteBootstrapCommand = buildRuntimeBunSuiteBootstrapCommand(
    consoleRunnerArgs,
    logDirInContainer,
    suiteMountTarget,
...
function runConsoleRunnerInContainer(config: ExportUpstreamConfig, suiteLocation: SuiteLocation): number {
  if (config.suiteDir) {
    return runRuntimeBunConsoleRunnerInContainer(config, suiteLocation);
  }
...
async function main(): Promise<number> {
  ...
  try {
    ...
    if (config.suiteDir) {
      // keep the active runtime tree in place
  ...
  if (!latestLogName) {
    const runLabel = config.suiteDir ? "Runtime" : "Upstream";
