...
import { getMigrationLedgerUnitByUnitKey } from "../../shared/execution/migration-ledger.ts";
...
const defaultRuntimeSuiteDir = resolve(repoRoot, "runtime");
...
export interface TraceDbStateBoundaryComparison {
  runtime: TraceDbStateBoundarySnapshot | null;
  different: boolean;
...
function usage(): string {
  return [
    "Usage:",
    "  bun ./legacy/scripts/traffic-diagnostic.ts [--version 1.0.3|2.0.0|all] [--compare-mode bag|ordered] [--db-state-mode none|unit|all|mutations] [--runtime-dir <path>] [--grep <pattern>] [--directory <csv>] [--unitKey <ledger-id>] [--target-base-url <url>] [--username <user>] [--password <pass>] [--out-dir <path>] [--oracle-dir <path>] [--refresh-upstream] [--keep-clone]",
    "",
    "Defaults:",
    "  --version all",
    "  --compare-mode bag",
...
function parseConfig(args: string[]): DiagnosticConfig {
  const runtimeDirArg = getFlagValue(args, "--runtime-dir");
  ...
  return {
    runtimeDir: runtimeDirArg
      ? isAbsolute(runtimeDirArg)
        ? runtimeDirArg
        : resolve(repoRoot, runtimeDirArg)
      : defaultRuntimeSuiteDir,
    compareMode,
    dbStateMode,
    directory,
    grep,
...
async function ensureLrsql(version: SupportedVersion): Promise<void> {
  ...
  await runRequiredCommandWithRetry("bash", ["./ops/scripts/lrsql-reset-best-effort.sh"], env, "LRSQL reset", {
    maxAttempts: 4,
    retryDelayMs: 1500,
  });
...
export async function assertRunArtifactExecutedTests(
  ...
  const runnerLabel = runner === "runtime" ? "Runtime" : "Upstream";
...
export function buildRuntimeArgs(config: DiagnosticConfig, endpoint: string, version: SupportedVersion): string[] {
  if (config.unitKeys) {
    return [
      "./legacy/scripts/export-upstream-run.ts",
      "--suite-dir",
      config.runtimeDir,
      "--base-url",
      endpoint,
      ...
      version,
      "--out",
      resolve(config.outDir, version, "runtime-run.json"),
      "--log-dir",
      resolve(config.outDir, version, "runtime-logs"),
      "--unitKey",
...
function shouldIgnoreTimingDrivenStatementPollMismatch(
  runtime: NormalizedTrafficArtifact,
...
export function suppressTimingDrivenStatementPollMismatches(
  ...
  const ignoredSignatureMismatches = comparison.signatureMismatches.filter((mismatch) =>
    shouldIgnoreTimingDrivenStatementPollMismatch(runtime, upstream, mismatch),
  );
...
function getTraceDbStateBoundarySortSequence(
  ...
  return Math.min(
    runtime?.rawSequenceEnd ?? Number.MAX_SAFE_INTEGER,
...
function haveEquivalentReplayIssues(runtime: TraceDbReplayIssue[], upstream: TraceDbReplayIssue[]): boolean {
...
export async function compareTraceDbStateManifests(options: {
  ...
  for (const boundaryKey of boundaryKeys) {
    ...
    let different = runtime === null || upstream === null;

    if (runtime && upstream) {
      if (
        !compareStringArrays(runtime.entryKinds, upstream.entryKinds) ||
        !compareStringArrays(runtime.nodeKeys, upstream.nodeKeys) ||
        !compareStringArrays(runtime.unitKeys, upstream.unitKeys)
      ) {
        different = true;
      }

      fingerprintComparison = compareFingerprints(
        await readFingerprintWithCache(runtime.fingerprintPath, fingerprintCache),
...
/**
 * Suppress boundary differences where the runtime and upstream `rawSequenceEnd` values
...
export function suppressUnalignedSequenceBoundaryDifferences(
  ...
  const alignedDivergent = report.divergentBoundaries.filter(
    (b) => b.runtime !== null && b.upstream !== null && b.runtime.rawSequenceEnd === b.upstream.rawSequenceEnd,
  );
...
function shouldSuppressSignedStatementBoundaryDifference(boundary: TraceDbStateBoundaryComparison): boolean {
  ...
  if (!boundary.runtime || !boundary.upstream || !comparison) {
    return false;
  }
...
function renderTraceDbStateComparisonReport(report: TraceDbStateComparisonReport): string {
  ...
  if (report.firstDivergentBoundary) {
    lines.push(
      ...
      `Runtime nodes: ${report.firstDivergentBoundary.runtime?.nodeKeys.join(", ") ?? "missing"}`,
    ...
    if (fingerprintComparison) {
      lines.push(
        `Only runtime tables: ${fingerprintComparison.onlyLeft.length}`,
...
async function captureRunner(
  ...
  try {
    const args =
      runner === "runtime"
        ? buildRuntimeArgs(config, recorder.captureBaseUrl, version)
...
async function runVersion(config: DiagnosticConfig, version: SupportedVersion): Promise<void> {
  ...
  await writeText(
    ...
    renderTrafficComparisonReport(stableRuntimeNormalized, stableUpstreamNormalized, comparison),
  );
