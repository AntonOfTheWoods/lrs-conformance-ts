...
function usage(): string {
  return [
    "Usage:",
    "  bun ./legacy/scripts/export-db-fingerprint.ts [--out <path>] [--compose-file <path>] [--service db] [--schema public] [--user <db-user>] [--database <db-name>] [--table-filter <regex>] [--exclude-table <name>] [--include-volatile-seed-tables]",
    "",
    "Defaults:",
    "  --out tmp/validation/oracles/db-fingerprint/<timestamp>.json",
    "  --compose-file ops/compose/lrsql/podman-compose.yml",
    "  --service db",
    "  --schema public",
...
function parseConfig(args: string[]): Config {
  ...
  const composeFile = getFlagValue(args, "--compose-file") ?? "ops/compose/lrsql/podman-compose.yml";
...
export async function exportDbFingerprint(options: ExportDbFingerprintOptions): Promise<FingerprintArtifact> {
  const config: Config = {
    composeFile: options.composeFile ?? "ops/compose/lrsql/podman-compose.yml",
