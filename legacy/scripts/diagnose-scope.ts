...
function usage(): string {
  return [
    "Usage:",
    "  bun ./legacy/scripts/diagnose-scope.ts [--upstream <path>] [--live <path>]",
    "",
    "Defaults:",
    "  --upstream tmp/validation/oracles/upstream-baselines/upstream-run-<version>.json",
    "  --live tmp/validation/oracles/runtime-baselines/full-lrsql-runtime-<version>.json",
  ].join("\n");
}
...
function getChildren(node: unknown): unknown[] {
  ...
  const runtime = node as { children?: unknown[]; tests?: unknown[] };
  if (Array.isArray(runtime.children)) {
    return runtime.children;
  }

  if (Array.isArray(runtime.tests)) {
...
async function main(): Promise<number> {
  ...
  const livePath =
    getFlagValue(args, "--live") ?? `tmp/validation/oracles/runtime-baselines/full-lrsql-runtime-${version}.json`;
  ...
  if (typeof upstreamPendingEstimate === "number" && upstreamPendingEstimate > 0) {
    console.error(
      `WARNING: upstream summary reports ${upstreamPendingEstimate} tests not represented as pass/fail. Comparison against full runtime tree will be misleading.`,
    );
    return 2;
  }
