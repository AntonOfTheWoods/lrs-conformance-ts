...
function discoverSiblingUpstreamRoot(projectRoot: string): string | null {
  ...
  for (const entry of readdirSync(parentDir, { withFileTypes: true })) {
    ...
    const runtime = join(parentDir, entry.name);
    if (isUpstreamSuiteRoot(runtime)) {
      return runtime;
    }
  }
...
export function resolveUpstreamTestRoot(version: "v1_0_3" | "v2_0"): string {
  return join(resolveUpstreamRoot(), "test", version);
}

export function resolveUpstreamPath(...segments: string[]): string {
  return join(resolveUpstreamRoot(), ...segments);
}
