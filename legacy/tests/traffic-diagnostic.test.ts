...
function createExecutionMetadata() {
  return createCaptureExecutionMetadata({
    ...
    sourceFilePath: "runtime/test/v1_0_3/Data2.2-FormattingRequirements.js",
...
function createStatementPollExecutionMetadata(caseName: string) {
  return createCaptureExecutionMetadata({
    ...
    sourceFilePath: "runtime/test/v1_0_3/E.Data2.5-RetrievalofStatements.js",
...
function createInvalidStatementParamExecutionMetadata(caseName: string) {
  return createCaptureExecutionMetadata({
    ...
    sourceFilePath: "runtime/test/v1_0_3/H.Communication2.1-StatementResource.js",
...
function createSignedStatementExecutionMetadata(caseName: string) {
  return createCaptureExecutionMetadata({
    ...
    sourceFilePath: "runtime/test/v1_0_3/E.Data2.6-SignedStatements.js",
...
test("buildRuntimeArgs targets the active runtime runtime tree", () => {
...
test("buildRuntimeArgs uses stable unitKey selection for single-unit migration runs", () => {
  const args = buildRuntimeArgs(
    {
      runtimeDir: "/tmp/runtime",
      compareMode: "ordered",
...
test("suppressTimingDrivenStatementPollMismatches ignores wait-loop retry drift for identical statement polls", () => {
  ...
  const runtime: NormalizedTrafficArtifact = {
  ...
  const comparison = compareNormalizedTrafficRuns(runtime, upstream, "bag");
  expect(comparison.signatureMismatches).toHaveLength(1);

  const effective = suppressTimingDrivenStatementPollMismatches(runtime, upstream, comparison);
...
test("stabilizeTimingDrivenStatementPolls strips volatile statement timestamps from repeated poll bursts", () => {
  ...
  const rawComparison = compareNormalizedTrafficRuns(runtime, upstream, "bag");
  expect(rawComparison.signatureMismatches.length).toBeGreaterThan(0);

  const comparison = compareNormalizedTrafficRuns(
    stabilizeTimingDrivenStatementPolls(runtime),
...
test("stabilizeSignedStatementAttachments strips volatile signature hashes from signed multipart requests", () => {
  ...
  const comparison = compareNormalizedTrafficRuns(
    stabilizeSignedStatementAttachments(runtime),
    ...
    divergentBoundaries: [
      {
        runtime: {
          entryKinds: ["case"],
          fingerprintPath: "/tmp/runtime-sequence-3.json",
          ...
          rowHashOrCountDifferences: [
            {
              left: {
                ...
                rowCount: 1,
                rowHash: "runtime-attachment-hash",
              },
              right: {
                columnNames: ["id", "statement_id", "attachment_sha", "content_type", "content_length", "contents"],
                rowCount: 1,
            ...
            },
            {
              left: {
                ...
                rowCount: 1,
                rowHash: "runtime-statement-hash",
              },
              right: {
                columnNames: [
                  "id",
    ...
    divergentBoundaries: [
      {
        runtime: {
          entryKinds: ["case"],
          fingerprintPath: "/tmp/runtime-sequence-1368.json",
          ...
          rowHashOrCountDifferences: [
            {
              left: {
                columnNames: ["id", "statement_id", "attachment_sha", "content_type", "content_length", "contents"],
                rowCount: 12,
                rowHash: "runtime-attachment-hash-1368",
              },
              right: {
                columnNames: ["id", "statement_id", "attachment_sha", "content_type", "content_length", "contents"],
                rowCount: 12,
            ...
            },
            {
              left: {
                ...
                rowCount: 451,
                rowHash: "runtime-statement-hash-1368",
              },
              right: {
                columnNames: [
                  "id",
      ...
      },
      {
        runtime: {
          entryKinds: ["case"],
          fingerprintPath: "/tmp/runtime-sequence-1758.json",
          ...
          rowHashOrCountDifferences: [
            {
              left: {
                columnNames: ["id", "statement_id", "attachment_sha", "content_type", "content_length", "contents"],
                rowCount: 16,
                rowHash: "runtime-attachment-hash-1758",
              },
              right: {
                columnNames: ["id", "statement_id", "attachment_sha", "content_type", "content_length", "contents"],
                rowCount: 16,
            ...
            },
            {
              left: {
                ...
                rowCount: 561,
                rowHash: "runtime-statement-hash-1758",
              },
              right: {
                columnNames: [
                  "id",
...
test("compareTraceDbStateManifests reports the first divergent boundary", async () => {
  ...
  try {
    ...
    const candidateSeq1Path = join(tempDir, "runtime-sequence-000001.json");
    const candidateSeq2Path = join(tempDir, "runtime-sequence-000002.json");
      ...
      `${JSON.stringify(
        {
          ...
          mode: "all",
          rawArtifactPath: join(tempDir, "runtime-raw.json"),
          replayIssues: [],
          runner: "runtime",
          schemaVersion: "trace-node-db-state-manifest.v1",
          selectedUnitKeys: [unitKey],
          traceNodeIndexPath: join(tempDir, "runtime-node-index.json"),
          version: "1.0.3",
        },
        null,
        2,
...
test("compareTraceDbStateManifests ignores raw-sequence shifts when node-aligned snapshots still match", async () => {
  ...
  try {
    ...
    const candidateCasePath = join(tempDir, "runtime-sequence-000012.json");
    const candidateHookPath = join(tempDir, "runtime-sequence-000014.json");
...
test("compareTraceDbStateManifests treats mirrored replay issues as non-divergent", async () => {
  ...
  try {
    ...
    const candidatePath = join(tempDir, "runtime-sequence-000018.json");
...
test("readEffectiveRunnerExitCode prefers the recorded upstream suite exit code", async () => {
  ...
  try {
    ...
    await writeFile(join(tempDir, "runtime-run.json"), `${JSON.stringify({ suiteExitCode: 3 })}\n`, "utf8");

    expect(await readEffectiveRunnerExitCode("upstream", tempDir, 0)).toBe(2);
    expect(await readEffectiveRunnerExitCode("runtime", tempDir, 0)).toBe(3);
  } finally {
    await rm(tempDir, { force: true, recursive: true });
  }
});
...
test("assertRunArtifactExecutedTests rejects run artifacts with null summary totals", async () => {
  ...
  try {
    const candidateRunPath = join(tempDir, "runtime-run.json");
    ...
    expect((thrown as Error).message).toContain("Runtime run did not execute test cases");
  } finally {
    await rm(tempDir, { force: true, recursive: true });
  }
});
