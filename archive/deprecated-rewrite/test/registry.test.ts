...
describe("Proof slice registry", () => {
  ...
  test("exposes the initial versioned registry tree for xAPI 1.0.3", () => {
    ...
    expect(v103Suites.length).toBe(25);
    expect(v103Suites.map((suite) => suite.id)).toEqual([
      ...
      "v1.proof-slice.statements.context",
      "v1.proof-slice.statements.representation",
      "v1.proof-slice.statements.special-data-types",
      "v1.proof-slice.statements.id",
      "v1.proof-slice.statements.metadata",
      "v1.proof-slice.statements.attachments",
      "v1.proof-slice.statements.authority",
      "v1.proof-slice.statements.signed-statements",
      "v1.proof-slice.statements.additional-data-types",
      "v1.proof-slice.statements.query-validation",
      ...
      "v1.proof-slice.agents-profile.validation-pack-a",
    ]);
  });
  ...
  test("includes the first migrated xAPI 1.0.3 requirement IDs", () => {
    ...
    expect(ids.size).toBe(314);
    expect(ids.has("XAPI-00001")).toBe(true);
    expect(ids.has("XAPI-00169")).toBe(true);
    expect(ids.has("XAPI-00335")).toBe(true);
