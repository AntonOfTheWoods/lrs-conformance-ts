...
export interface MigrationLedgerUnit {
  directory: string;
  mode: MigrationMode;
  runtimeRegistrarFilePath: string | null;
  runtimeRegistrarSymbol: string | null;
  unitKey: string;
...
type UnitSpec = readonly [
  ...
  runtimeRegistrarFilePath: string,
  runtimeRegistrarSymbol: string,
  version: "1.0.3" | "2.0.0" | null,
];

const activeRegistryModes = new Set<MigrationMode>(["dual-run-compare", "runtime-active"]);
const observedUnitByDirectoryAndTitle = new Map<string, MigrationLedgerUnit>();

function defineUnit(
  directory: string,
  [upstreamFileStem, runtimeRegistrarFilePath, runtimeRegistrarSymbol, version]: UnitSpec,
): MigrationLedgerUnit {
  const normalizedRegistrarFilePath = runtimeRegistrarFilePath.startsWith("src/specs/")
    ? runtimeRegistrarFilePath.replace("src/specs/", "test/")
    : runtimeRegistrarFilePath;

  return {
    directory,
    mode: "runtime-active",
    runtimeRegistrarFilePath: `${runtimeRoot}/${normalizedRegistrarFilePath}`,
    runtimeRegistrarSymbol,
...
export function getMigrationLedgerUnitsForDirectory(directory: string): readonly MigrationLedgerUnit[] {
  return migrationLedger.units.filter(
    (unit) => unit.directory === directory && activeRegistryModes.has(unit.mode) && unit.runtimeRegistrarSymbol,
  );
}
...
export function resolveMigrationUnitBinding(directory: string, suitePath: string[]): MigrationUnitBinding {
  ...
  if (observedUnit) {
    return {
      sourceFilePath: observedUnit.runtimeRegistrarFilePath,
      sourceSymbol: observedUnit.runtimeRegistrarSymbol,
