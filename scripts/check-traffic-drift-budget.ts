...
function main(): void {
  ...
  if (!comparePathArg) {
    console.error(
      "Usage: bun ./scripts/check-traffic-drift-budget.ts --compare <compare.json> [--suite-failures <suite-failure-parity.json>] [--allowlist <allowlist.json>] [--max-mismatches <n>]",
    );
    process.exitCode = 1;
    return;
  }
