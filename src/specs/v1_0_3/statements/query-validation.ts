...
import { statementsQueryValidationSuite } from "../../v2_0/statements/areas/query-validation";
...
export function createV103StatementsQueryValidationProofSliceSuite(): SuiteDefinition {
  const adapted = rewriteNodeFromV2(statementsQueryValidationSuite as unknown as RegistryNode);
  if (adapted.type !== "suite") {
    throw new Error("expected statements query-validation root to be a suite");
  }

  adapted.id = "v1.proof-slice.statements.query-validation";
  adapted.title = "Statements Query Validation";
  adapted.tags = ["proof-slice", "statements", "query-validation"];
  return adapted;
}
