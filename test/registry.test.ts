...
import { createProofSliceRegistry, createV20ProofSliceSuite } from "../src/specs/v2_0/proof-slice";
...
describe("RegistryBuilder", () => {
  ...
  test("builds a stable manifest for the first 2.0 proof slice", () => {
    ...
    expect(manifest.versions["2.0.0"].suiteCount).toBe(1);
    expect(manifest.versions["2.0.0"].caseCount).toBe(4);
  ...
  test("compiles a batteries-like artifact without executing the suite", () => {
    ...
    expect(batteries["2.0.0"]?.conformanceTestCount).toBe(4);
...
describe("Proof slice registry", () => {
  test("exposes a versioned registry tree for xAPI 2.0.0", () => {
    ...
    expect(registry.versions["2.0.0"]).toHaveLength(1);
