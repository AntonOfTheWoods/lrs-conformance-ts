...
function usage(): string {
  return [
    "Usage:",
    "  bun ./rewrite/scripts/bisect-db-divergence.ts --left-artifact <rewrite-raw.json> --right-artifact <upstream-raw.json> --version <1.0.3|2.0.0> [--max-prefix <N>] [--target-base-url <url>] [--compose-file <path>] [--out <path>]",
  ].join("\n");
}
...
function parseConfig(args: string[]): Config {
  ...
  return {
    composeFile: getFlagValue(args, "--compose-file") ?? "compose/lrsql/podman-compose.yml",
    leftArtifactPath: resolve(leftArtifactPath),
    maxPrefix,
    outPath: resolveSafeArtifactPath(
      getFlagValue(args, "--out") ?? resolve(repoRoot, "tmp/agents/db-bisect", `${Date.now()}.json`),
    ),
...
async function probePrefix(
  ...
  await runRequiredCommand(
    "bun",
    [
      "./rewrite/scripts/replay-traffic-prefix.ts",
      "--artifact",
  ...
  await runRequiredCommand(
    "bun",
    ["./rewrite/scripts/export-db-fingerprint.ts", "--compose-file", config.composeFile, "--out", leftFingerprintPath],
    createVersionEnvironment(config.version),
  ...
  await runRequiredCommand(
    "bun",
    ["./rewrite/scripts/export-db-fingerprint.ts", "--compose-file", config.composeFile, "--out", rightFingerprintPath],
    createVersionEnvironment(config.version),
